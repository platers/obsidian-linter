...
export default class LinterPlugin extends Plugin {
  ...
  async runLinterAllFilesInFolder(folder: TFolder) {
    ...
    if (numberOfErrors === 0) {
      new Notice(getTextInLanguage('commands.lint-all-files-in-folder.success-message').replace('{NUM}', lintedFiles.toString()).replace('{FOLDER_NAME}', folder.name));
    } else {
