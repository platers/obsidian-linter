...
export default {
  commands: {
    ...
    "lint-all-files-in-folder": {
      name: "格式化目前資料夾中的所有檔案",
      "start-message":
        "這將會編輯您在 {FOLDER_NAME} 中的所有檔案，包含其子資料夾中的檔案，這可能會導致錯誤。",
      ...
      "message-singular":
        "已格式化 {FOLDER_NAME} 中的所有 {NUM} 個檔案，但有 1 個錯誤。",
      "message-plural":
        "已格式化 {FOLDER_NAME} 中的所有 {FILE_COUNT} 個檔案，但有 {ERROR_COUNT} 個錯誤。",
    },
    "paste-as-plain-text": {
      name: "以純文字貼上且不做修改",
    },
  ...
  logs: {
    ...
    "file-change-yaml-lint-warning":
      "沒有檔案資訊，但 debounce 已執行。某處發生錯誤。",
    "paste-link-warning":
      "已中止貼上格式化，因為剪貼簿內容是連結，這樣做可以避免與其他修改貼上行為的外掛發生衝突。",
    "see-console": "詳情請見主控台。",
    "unknown-error": "格式化期間發生未知錯誤。",
    "moment-locale-not-found":
      "嘗試將 Moment.js 地區設定切換為 {MOMENT_LOCALE}，但得到 {CURRENT_LOCALE}",
    ...
    "custom-lint-duplicate-warning":
      '您不能將相同的命令（"{COMMAND_NAME}"）作為自訂格式化規則執行兩次。',
    ...
    "timing-key-not-found":
      "計時金鑰 '{TIMING_KEY}' 不存在於計時資訊清單中，因此被忽略",
  ...
  "custom-command-warning":
    "使用自訂命令格式化多個檔案是一個緩慢的過程，需要在側邊面板中開啟窗格。它的速度明顯慢於在未啟用自訂命令的情況下執行。請謹慎操作。",
  "cancel-button-text": "取消",

  "copy-aria-label": "複製",

  "disabled-other-rule-notice":
    "如果您啟用 <code>{NAME_1}</code>，它將停用 <code>{NAME_2}</code>。您要繼續嗎？",
  "disabled-conflicting-rule-notice":
    "{NAME_1} 與 {NAME_2} 衝突，因此已被關閉。您可以在設定分頁中切換要關閉的設定。",
  ...
  "no-parsed-values-found-text":
    "在 {FILE} 中找不到自訂取代項目。請確保 {FILE} 中所有具有自訂取代項目的表格都只有兩欄，且所有列都以管道符號（即 |）開頭和結尾。",
  ...
  tabs: {
    ...
    general: {
      ...
      "lint-on-save": {
        name: "儲存時格式化",
        description:
          "在手動儲存檔案時格式化檔案（當按下 <code>Ctrl + S</code> 或在使用 vim 按鍵綁定時執行 <code>:w</code>）",
      },
      "display-message": {
        name: "格式化時顯示訊息",
      ...
      "remove-unnecessary-escape-chars-in-multi-line-arrays": {
        ...
        description:
          "多行 YAML 陣列的逸出字元不需要與單行陣列相同的逸出，因此在多行格式中移除不必要的額外逸出",
      },
    ...
    debug: {
      ...
      "log-collection": {
        ...
        description:
          "在您 <code>儲存時格式化</code> 和格式化目前檔案時收集日誌。這些日誌有助於偵錯和建立錯誤報告。",
      },
      "linter-logs": {
        name: "Linter 日誌",
        description:
          "如果啟用，則為上次 <code>儲存時格式化</code> 或上次格式化目前檔案執行的日誌。",
      },
    },
  },
  ...
  options: {
    "custom-command": {
      ...
      name: "自訂命令",
      description:
        "自訂命令是在 Linter 完成其一般規則後執行的 Obsidian 命令。這表示它們不會在 YAML 時間戳記邏輯執行之前執行，因此它們可能會導致 YAML 時間戳記在下一次 linter 執行時觸發。您只能選擇一個 Obsidian 命令一次。",
      warning:
        "選擇選項時，請務必使用滑鼠或按 Enter 鍵選擇選項。其他選擇方法可能無效，只有選擇實際的 Obsidian 命令或空字串才會被儲存。",
    ...
    "custom-replace": {
      ...
      name: "自訂正規表示式取代",
      description:
        "自訂正規表示式取代可用於將任何符合尋找正規表示式的內容取代為取代值。取代和尋找值都必須是有效的正規表示式值。",
      warning:
        "如果您不了解正規表示式，請謹慎使用。此外，請確保如果您在 iOS 行動裝置上的正規表示式中使用 lookbehind，您使用的版本支援它們。",
  ...
  rules: {
    ...
    "auto-correct-common-misspellings": {
      name: "自動更正常見錯字",
      description:
        '使用常見錯字字典自動將其轉換為正確的拼寫。請參閱 <a href="https://github.com/platers/obsidian-linter/tree/master/src/utils/default-misspellings.md">自動更正對應表</a> 以取得自動更正單字的完整清單。<b>注意：此清單適用於多種語言的文字，但無論目前使用何種語言，此清單都相同。</b>',
      "ignore-words": {
        name: "忽略單字",
      ...
      "extra-auto-correct-files": {
        name: "額外自動更正來源檔案",
        description:
          "這些檔案中包含一個 Markdown 表格，其中包含初始單字和要更正的單字（這些是不區分大小寫的更正）。<b>注意：使用的表格應在每一行都包含開始和結束的 <code>|</code> 指示符。</b>",
      },
      "skip-words-with-multiple-capitals": {
        name: "跳過具有多個大寫字母的單字",
        description:
          "將跳過任何除了單字第一個字母外還包含大寫字母的檔案。縮寫和其他一些單字可以從中受益。這可能會導致專有名詞無法正確修正的問題。",
      },
      "default-install":
        "您正在使用自動更正常見錯字。為此，將下載預設的錯字。這應該只會發生一次。請稍候...",
      "default-install-failed": "下載 {URL} 失敗。正在停用自動更正常見錯字。",
    ...
    "add-blank-line-after-yaml": {
      name: "在 YAML 後新增空白行",
      description:
        "在 YAML 區塊後新增一個空白行，如果它不是目前檔案的結尾，或者它後面還沒有至少一個空白行",
    },
    // blockquotify-on-paste.ts
    "add-blockquote-indentation-on-paste": {
      name: "貼上時新增區塊引言縮排",
      description:
        "在貼上期間，當游標位於區塊引言/引文行時，為除第一行外的所有行新增區塊引言",
    },
    // blockquote-style.ts
    "blockquote-style": {
      name: "區塊引言樣式",
    ...
    "dedupe-yaml-array-values": {
      ...
      "ignore-keys": {
        name: "要忽略的 YAML 鍵",
        description:
          "一個 YAML 鍵的清單，每行一個，不含結尾的冒號，這些鍵不應移除重複值。",
      },
    },
    ...
    "default-language-for-code-fences": {
      ...
      "default-language": {
        name: "程式語言",
        description:
          '留空則不執行任何操作。語言標籤可在此處找到 <a href="https://prismjs.com/#supported-languages">這裡</a>。',
      },
    },
    // emphasis-style.ts
    "emphasis-style": {
    ...
    "empty-line-around-blockquotes": {
      name: "區塊引言周圍的空白行",
      description:
        "確保區塊引言周圍有空白行，除非它們在文件的開頭或結尾。<b>注意：空白行可以是少一層巢狀的區塊引言，也可以是換行字元。</b>",
    },
    ...
    "empty-line-around-math-blocks": {
      name: "數學區塊周圍的空白行",
      description:
        "確保數學區塊周圍有空白行，使用 <code>表示數學區塊的錢字號數量</code> 來決定多少個錢字號表示單行數學的數學區塊。",
    },
    // empty-line-around-tables.ts
    "empty-line-around-tables": {
      name: "表格周圍的空白行",
      description:
        "確保 github 風格的表格周圍有空白行，除非它們在文件的開頭或結尾。",
    },
    // escape-yaml-special-characters.ts
    "escape-yaml-special-characters": {
      name: "逸出 YAML 特殊字元",
      description:
        "在 YAML 中逸出後面有空格的冒號（: ）、單引號（'）和雙引號（\"）。",
      "try-to-escape-single-line-arrays": {
        name: "嘗試逸出單行陣列",
        description:
          "假設陣列以 '[' 開頭，以 ']' 結尾，且項目以 ',' 分隔，嘗試逸出陣列值。",
      },
    },
    ...
    "force-yaml-escape": {
      ...
      "force-yaml-escape-keys": {
        name: "在鍵上強制 YAML 逸出",
        description:
          "在指定的 YAML 鍵上使用 YAML 逸出字元，以換行字元分隔，如果尚未逸出。請勿在 YAML 陣列上使用。",
      },
    },
    ...
    "format-tags-in-yaml": {
      name: "格式化 YAML 中的標籤",
      description:
        "從 YAML frontmatter 中的標籤移除井字號，因為它們會使那裡的標籤無效。",
    },
    // format-yaml-array.ts
    "format-yaml-array": {
      name: "格式化 YAML 陣列",
      description:
        "允許將一般 YAML 陣列格式化為多行或單行，且 <code>tags</code> 和 <code>aliases</code> 允許使用一些 Obsidian 特定的 YAML 格式。<b>注意：單一字串到單行是從單一字串項目變為單行陣列，如果存在多於一個項目。單一字串到多行也是如此，只是它會變成多行陣列。</b>",
      "alias-key": {
        name: "格式化 YAML 別名區段",
        description:
          "開啟 YAML 別名區段的格式化功能。您不應將此選項與 <code>YAML 標題別名</code> 規則一起啟用，因為它們可能無法很好地協同工作，或者它們可能選擇了不同的格式樣式，導致意外結果。",
      },
      "tag-key": {
        name: "格式化 YAML 標籤區段",
      ...
      "default-array-style": {
        name: "預設 YAML 陣列區段樣式",
        description:
          "其他 YAML 陣列的樣式，這些陣列不是 <code>tags</code>、<code>aliases</code> 或在 <code>強制鍵值為單行陣列</code> 和 <code>強制鍵值為多行陣列</code> 中的陣列",
      },
      "default-array-keys": {
        name: "格式化 YAML 陣列區段",
      ...
      "force-single-line-array-style": {
        name: "強制鍵值為單行陣列",
        description:
          "強制以換行分隔的鍵的 YAML 陣列為單行格式（留空以停用此選項）",
      },
      "force-multi-line-array-style": {
        name: "強制鍵值為多行陣列",
        description:
          "強制以換行分隔的鍵的 YAML 陣列為多行格式（留空以停用此選項）",
      },
    },
    // header-increment.ts
    "header-increment": {
      ...
      "start-at-h2": {
        name: "從標題層級 2 開始標題遞增",
        description:
          "將標題層級 2 設為檔案中標題遞增的最低標題層級，並相應地移動所有標題，使其從層級 2 標題開始遞增。",
      },
    },
    ...
    "headings-start-line": {
      name: "標題起始行",
      description:
        "不以行開頭的標題將移除其前面的空白，以確保它們被辨識為標題。",
    },
    // insert-yaml-attributes.ts
    "insert-yaml-attributes": {
      name: "插入 YAML 屬性",
      description:
        "將給定的 YAML 屬性插入到 YAML frontmatter 中。將每個屬性放在單獨的一行。",
      "text-to-insert": {
        name: "要插入的文字",
    ...
    "move-footnotes-to-the-bottom": {
      name: "將註腳移至底部",
      description:
        "將所有註腳移至文件底部，並確保它們根據在檔案內文中參考的順序進行排序。",
    },
    // move-math-block-indicators-to-their-own-line.ts
    "move-math-block-indicators-to-their-own-line": {
      name: "將數學區塊指示符移至其自己的行",
      description:
        "使用 <code>表示數學區塊的錢字號數量</code> 來決定多少個錢字號表示單行數學的數學區塊，將所有起始和結束的數學區塊指示符移至其自己的行。",
    },
    ...
    "move-tags-to-yaml": {
      ...
      "tags-to-ignore": {
        name: "要忽略的標籤",
        description:
          "如果啟用 <code>從內容內文中移除標籤的井字號</code>，則不會移至標籤陣列或從內文內容中移除的標籤。每個標籤應在新的一行，且不含 <code>#</code>。<b>請確保標籤名稱中不包含井字號。</b>",
      },
    },
    // no-bare-urls.ts
    "no-bare-urls": {
      name: "無裸 URL",
      description:
        "用角括號括住裸 URL，除非它們被反引號、方括號或單引號或雙引號括住。",
      "no-bare-uris": {
        name: "無裸 URI",
        description:
          "嘗試用角括號括住裸 URI，除非它們被反引號、方括號或單引號或雙引號括住。",
      },
    },
    ...
    "ordered-list-style": {
      name: "有序清單樣式",
      description:
        "確保有序清單遵循指定的樣式。<b>注意：2 個空格或 1 個定位點被視為一個縮排層級。</b>",
      "number-style": {
        name: "數字樣式",
      ...
      "preserve-start": {
        name: "保留起始編號",
        description:
          "是否保留有序清單的起始編號。這可用於在有序清單項目之間有內容的有序清單。",
      },
    },
    ...
    "prevent-double-checklist-indicator-on-paste": {
      name: "貼上時防止雙重清單指示符",
      description:
        "如果檔案中游標所在的行有清單指示符，則從要貼上的文字中移除起始清單指示符",
    },
    // prevent-double-list-item-indicator-on-paste.ts
    "prevent-double-list-item-indicator-on-paste": {
      name: "貼上時防止雙重清單項目指示符",
      description:
        "如果檔案中游標所在的行有清單項目指示符，則從要貼上的文字中移除起始清單項目指示符",
    },
    // proper-ellipsis-on-paste.ts
    "proper-ellipsis-on-paste": {
      name: "貼上時使用正確的刪節號",
      description:
        "將三個連續的點取代為一個刪節號，即使它們在要貼上的文字中有空格",
    },
    // proper-ellipsis.ts
    "proper-ellipsis": {
      name: "正確的刪節號",
    ...
    "re-index-footnotes": {
      name: "重新索引註腳",
      description:
        "根據出現順序重新索引註腳鍵和註腳。<b>注意：如果一個鍵有多個註腳，此規則將不起作用。</b>",
    },
    ...
    "remove-horizontal-rules-outside-yaml": {
      name: "移除 YAML 外的水平線",
      description:
        "從文件主體內容中移除水平線分隔符（`---`），同時保留 YAML frontmatter 邊界。",
    },
    ...
    "remove-space-around-characters": {
      name: "移除字元周圍的空格",
      description:
        "確保某些字元不被空白（單個空格或定位點）包圍。<b>注意：在某些情況下，這可能會導致 Markdown 格式出現問題。</b>",
      "include-fullwidth-forms": {
        name: "包含全形字元",
        description:
          '包含 <a href="https://en.wikipedia.org/wiki/Halfwidth_and_Fullwidth_Forms_(Unicode_block)">全形字元 Unicode 區塊</a>',
      },
      "include-cjk-symbols-and-punctuation": {
        name: "包含中日韓符號和標點符號",
        description:
          '包含 <a href="https://en.wikipedia.org/wiki/CJK_Symbols_and_Punctuation">中日韓符號和標點符號 Unicode 區塊</a>',
      },
      "include-dashes": {
        name: "包含破折號",
    ...
    "remove-space-before-or-after-characters": {
      name: "移除字元之前或之後的空格",
      description:
        "移除指定字元之前的空格和指定字元之後的空格。<b>注意：在某些情況下，這可能會導致 Markdown 格式出現問題。</b>",
      "characters-to-remove-space-before": {
        name: "移除字元之前的空格",
        description:
          "移除指定字元之前的空格。<b>注意：在字元清單中使用 <code>{</code> 或 <code>}</code> 會意外地影響檔案，因為它在幕後用於忽略語法。</b>",
      },
      "characters-to-remove-space-after": {
        name: "移除字元之後的空格",
        description:
          "移除指定字元之後的空格。<b>注意：在字元清單中使用 <code>{</code> 或 <code>}</code> 會意外地影響檔案，因為它在幕後用於忽略語法。</b>",
      },
    },
    ...
    "remove-trailing-punctuation-in-heading": {
      name: "移除標題中的尾隨標點符號",
      description:
        '從標題結尾移除指定的標點符號，確保忽略 <a href="https://en.wikipedia.org/wiki/List_of_XML_and_HTML_character_entity_references">HTML 實體參考</a> 結尾的分號。',
    ...
    "sort-yaml-array-values": {
      ...
      "ignore-keys": {
        name: "要忽略的 YAML 鍵",
        description:
          "一個 YAML 鍵的清單，每行一個，不含結尾的冒號，這些鍵的值不應進行排序。",
      },
      "sort-order": {
        name: "排序順序",
    ...
    "space-between-chinese-japanese-or-korean-and-english-or-numbers": {
      ...
      description:
        '確保中日韓文與英文或數字之間以單一空格分隔。遵循這些 <a href="https://github.com/sparanoid/chinese-copywriting-guidelines">指南</a>',
      "english-symbols-punctuation-before": {
        name: "中日韓文前的英文標點符號和符號",
        description:
          "在中日韓字元前發現時，要視為英文的非字母標點符號和符號清單。<b>注意：「*」始終被視為英文，對於正確處理某些 Markdown 語法是必要的。</b>",
      },
      "english-symbols-punctuation-after": {
        name: "中日韓文後的英文標點符號和符號",
        description:
          "在中日韓字元後發現時，要視為英文的非字母標點符號和符號清單。<b>注意：「*」始終被視為英文，對於正確處理某些 Markdown 語法是必要的。</b>",
      },
    },
    // strong-style.ts
    "strong-style": {
    ...
    "two-spaces-between-lines-with-content": {
      name: "有內容的行之間的換行",
      description:
        "確保將指定的換行符新增到段落、區塊引言和清單項目中，內容在下一行繼續的行的結尾",
    ...
    "yaml-key-sort": {
      name: "YAML 鍵排序",
      description:
        "根據指定的順序和優先級對 YAML 鍵進行排序。<b>注意：也可能移除空白行。僅適用於非巢狀鍵。</b>",
      "yaml-key-priority-sort-order": {
        name: "YAML 鍵優先級排序順序",
        description:
          "對每行一個的鍵進行排序的順序，它會按照清單中找到的順序進行排序",
      },
      ...
      "yaml-sort-order-for-other-keys": {
        name: "其他鍵的 YAML 排序順序",
        description:
          "對在 YAML 鍵優先級排序順序文字區域中找不到的鍵進行排序的方式",
      },
    },
    // yaml-timestamp.ts
    "yaml-timestamp": {
      name: "YAML 時間戳記",
      description:
        "在 YAML front matter 中追蹤檔案上次編輯的日期。從檔案元資料中取得日期。",
      "date-created": {
        name: "建立日期",
      ...
      "date-created-source-of-truth": {
        name: "建立日期來源",
        description:
          "指定如果 frontmatter 中已存在建立日期，則從何處取得建立日期值。",
      },
      "date-modified-source-of-truth": {
        name: "修改日期來源",
        description:
          "指定如果 frontmatter 中已存在修改日期，則應使用何種方式來決定何時應更新修改日期。",
      },
      "date-modified": {
        name: "修改日期",
      ...
      format: {
        name: "格式",
        description:
          '要使用的 Moment 日期格式（請參閱 <a href="https://momentjscom.readthedocs.io/en/latest/moment/04-displaying/01-format/">Moment 格式選項</a>）',
      },
      "convert-to-utc": {
        name: "將本地時間轉換為 UTC",
      ...
      "update-on-file-contents-updated": {
        ...
        description:
          "當目前活動的筆記被修改時，會在筆記上執行 <code>YAML 時間戳記</code>。如果修改後的筆記時間戳記與目前值相差超過 5 秒，則應更新該時間戳記。",
      },
    },
    // yaml-title-alias.ts
    "yaml-title-alias": {
      name: "YAML 標題別名",
      description:
        "將檔案的標題插入或更新到 YAML frontmatter 的別名區段。從第一個 H1 或檔名取得標題。",
      "preserve-existing-alias-section-style": {
        name: "保留現有的別名區段樣式",
        description:
          "如果設定，<code>YAML 別名區段樣式</code> 設定僅適用於新建立的區段",
      },
      ...
      "use-yaml-key-to-keep-track-of-old-filename-or-heading": {
        ...
        description:
          "如果設定，當第一個 H1 標題變更或如果第一個 H1 不存在時檔名變更，則儲存在此鍵中的舊別名將被新值取代，而不是僅在別名陣列中插入新項目",
      },
      "alias-helper-key": {
        name: "別名輔助鍵",
        description:
          "用於協助追蹤此規則儲存在 Frontmatter 中的最後一個檔名或標題的鍵。",
      },
    },
    // yaml-title.ts
    "yaml-title": {
      name: "YAML 標題",
      description:
        "將檔案的標題插入到 YAML Frontmatter 中。根據所選模式取得標題。",
      "title-key": {
        name: "標題鍵",
